...
var getPayData = require('./customfunctions.js').getPayData;
...
module.exports.getNewProductPage = function(req, res){
	var data = {
		layout: 'main',
		product: []
	};
	async.parallel([
		...
		function(callback) {
			Product.find({isNewProduct: true}).sort({rank: 1}).exec(function(err, rep){
				if(rep) data.product = rep;
				callback();
			});
		}
		], function(err) { 
			res.render('pages/newproductpage', data);
	});	
};
...
module.exports.getStaticPage = function(req, res){
	...
	// To avoid require ext in View function of express render
...
module.exports.getProductsList = function(req, res){
	...
	if (counter!==undefined) { /*check if filter for categories was used*/
		...
		categories.forEach(function(category){
			Product.find({'category.id': category}, 'name address quantity isHit isNewProduct image_micro_thumb category', function(err, rep){
		...
		Product.find({isHit: true}, 'name address quantity isHit isNewProduct image_micro_thumb category', function(err, rep){
...
module.exports.getPayPage = function(req,res){
	var num = req.params.num;
	var data={layout:'main',paydata:{},notFound:false};
	...
	async.parallel([
		...
		function(callback) {
		Order.findOne({number:Number(num),isDone:false},function(err,ord){
			if(ord)									
				data.payData = getPayData(ord);
			else
				data.notFound = true;
			callback();		
		})
			...
			if(!err && !data.notFound)
				res.render('pages/pay/pay',data);
			else 
				res.render('pages/pay/notfound',data);
	});
...
module.exports.getSuccessPage = function(req,res){
	var data = {
		layout: 'main'
	};
	getCommonData(data, function(err, rep){
		res.render('pages/pay/success',data);		
	});
};

module.exports.getFailPage = function(req,res){
	var data = {
		layout: 'main'
	};
	getCommonData(data, function(err, rep){
		res.render('pages/pay/fail',data);
	});	
};
