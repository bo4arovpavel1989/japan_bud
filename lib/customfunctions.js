...
module.exports.getCommonData = function(data, fnCallback) {
	async.parallel([
		...
		function(callback){
			fs.readdir(__dirname +'/../public/carousel/images',function(err, names){
				if(names) data.carousel = names;
				callback();
			})
		}
		], function(err) { 
					...
					if (index === -1) { /*current product doesnt dublicate, so i add it to the cart and to the control array*/
						...
						for(var i=0; i < data.product.length; i++) {
							if (rep.name === data.product[i].name) {
								...
								data.product[i].sumWeight += rep.weight;
...
module.exports.getOrderData = function(req){
	var data = {
		product: []
	};
	...
	data.fedexPrice = req.body.fedexPrice;
	data.date = Date.now();
	
	return data
}

module.exports.getProductData = function (data, rep){
	if (rep.isSpecialProp) 
		data.product.forEach(function(productItem){
			if(productItem.name == rep.name) {
				productItem.price = rep.discount; 
				productItem.weight = rep.weight;
...
module.exports.getTotalWeightAndPrice = function(data,totalWeight,totalPrice){
	data.product.forEach(function(productItem){
		totalPrice += (Number(productItem.price) * Number(productItem.howMany));
		totalWeight += (Number(productItem.weight) * Number(productItem.howMany));
	});
	
	data.totalPrice = totalPrice;
	
	if(!isNaN(totalPrice)&&!isNaN(data.fedexPrice))
		data.toPay = Number(totalPrice) + Number(data.fedexPrice);
...
var generateSign = function(sum, num){
	var str = secret.robokassa.sMerchantLogin + ':' + sum + ':' + num + ':' +  secret.robokassa.sMerchantPass1;
	return md5(str);
}

module.exports.getPayData = function(ord){
	return {
			layout:'main',
			login:secret.robokassa.sMerchantLogin,
			sum: ord.toPay,
			num: ord.number,
			email: ord.email,
			desc:'order'+ord.number,
			sign: generateSign(ord.toPay,ord.number)
	}
}